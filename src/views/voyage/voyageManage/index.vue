<template>
  <div class="app-container">
      ...
      <el-table v-loading="loading" :data="tableData" style="width: 100%" :border="true" 
          ...
          <template #default="scope">
            <template v-if="scope.row.edit">
              <div style="display: flex; align-items: center">
                <el-date-picker
                  v-model="scope.row.voyDate" type="date" placeholder="请选择日期" :clearable="false"
                  format="YYYY-MM-DD" 
        ...
        <el-table-column prop="reShipName" label="实际船名" width="150px" align="center" >
          <template #default="scope">
            <template v-if="scope.row.edit">
              <el-select
                v-model="scope.row.reShipNo"
                filterable  
                placeholder="请选择或输入实际船"
                @click="queryAllShipNames"
              >
              <el-option v-for="ship in ships" :label="ship.shipName" :value="ship.shipNo"></el-option>
              </el-select>
        ...
        <el-table-column prop="startTime" label="出发时间" sortable width="150px" align="center">
          <template #default="scope">
            <template v-if="scope.row.edit">
              <div style="display: flex; align-items: center">
              <el-time-picker v-model="scope.row.startTime" arrow-control
                format="HH:mm:ss" value-format="HH:mm:ss" :clearable="false"
                placeholder="请选择出发时间">
              </el-time-picker>
              </div>
            </template>
        ...
        <el-table-column prop="vipRes" label="VIP预留" width="140px" align="center">
          <template #default="scope">
            <template v-if="scope.row.edit">
              <div style="display: flex; align-items: center">
                  <el-input-number :precision="0" :min="0" v-model="scope.row.vipRes"></el-input-number>
              </div>
            </template>
          </template>
        </el-table-column>
        <el-table-column prop="firRes" label="一等预留" width="140px" align="center">
          <template #default="scope">
            <template v-if="scope.row.edit">
              <div style="display: flex; align-items: center">
                  <el-input-number :precision="0" :min="0" v-model="scope.row.firRes"></el-input-number>
              </div>
            </template>
          </template>
        </el-table-column>
        <el-table-column prop="secRes" label="二等预留" width="140px" align="center">
          <template #default="scope">
            <template v-if="scope.row.edit">
              <div style="display: flex; align-items: center">
                <el-input-number  :precision="0" :min="0" v-model="scope.row.secRes"></el-input-number>
              </div>
            </template>
          </template>
        </el-table-column>
    ...
    <VoyageDialog ref="voyageDialog" :getData="getData"/>
    <VopyVoysDialog ref="copyVoysDialog" :getData="getData"/>
  </div>
</template>
