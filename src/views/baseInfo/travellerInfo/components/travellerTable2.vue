<template>
  <div ref="appContainer" class="app-container">
    <div class="header">
      ...
      <el-button @click="importExcelAction" type="primary">
    ...
    <TravellerDialog ref="travellerDialog" :getData="getData"/>    
  </div>
</template>
          ...
          sheets.forEach((sheet) => {
            ...
            if (rowNumber === 1) {
              ...
              headers.forEach((header, i) => {
                switch(header){
                  case "护照号": header = 'passportNo';break;
                  case "姓名": header = 'trName';break;
                  case "英文名": header = 'trPinYin';break;
                  case "手机号": header = 'trPhone';break;
                  case "临时手机号": header = 'trTmpPhone';break;
                  case "性别": header = 'trSex';break;
                  case "出生日期": header = 'trBirthday';break;
                  case "出生地": header = 'trBorAdd';break;
                  case "国籍": header = 'trNation';break;
                  case "旅行社": header = 'trTravel';break;
                }
                if (i < row.values.length) {
                    let cellValue = row.values[i];
                    // 如果单元格的值是一个包含文本的对象(意思是不管excel单元数据的格式)
                    if (typeof cellValue === 'object' && cellValue.hasOwnProperty('richText')) {
                        // 提取文本对象中的文本值
