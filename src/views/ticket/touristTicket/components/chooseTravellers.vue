<template>
        ...
        <el-table-column label="船名信息" align="center">
          <template #default="scope">
            <el-popover effect="light" trigger="hover" placement="top" align="center" width="auto">
              ...
              <template #reference>
                <div>
                  <el-tag size="large" round type="light" style="color: #606266; font-weight: bold;font-size: 20px">{{ scope.row.shipName }}</el-tag>
                </div>
              </template>
            </el-popover>
          </template>
    ...
    <div class="footer">
      ...
      <div class="table-inner" style="display: flex;height: 440px">
        <el-table v-loading="loading" :data="tableData" 
          style="width: 50%; height: 100%" border
          :row-key="getRowKey"
        ...
        <el-table v-loading="loading" :data="chooseTravellers"
          :row-key="getRowKey"
          style="width: 40%; height: 100%; margin-left: auto" border>
          <el-table-column type="index" fixed label="序号" align="center" width="55">
            <template #default="{ $index }">
                {{ getIndex2($index) }}
            </template>
      ...
      <div v-if="chooseTravellers.length !== 0" style="display: flex; width: 220px; flex-direction: column; align-items: flex-end; margin-left:auto;">
    ...
    :chooseTravellers="chooseTravellers" :voyData="voyData" 
    :getTotalMoney="getTotalMoney" :generateOrder="generateOrder"/>
</template>
...
<script setup lang="ts">
  ...
  const { voyData, nextStep, orderData } = defineProps({
    voyData: {
      type: Array
    },
    nextStep: {
  ...
  const condition = reactive({
    ...
    passportNo: '',
    trName: '',
    voyTime: voyData[0]['voyDate'] + ' ' + voyData[0]['startTime']
  })
  ...
  const tickCost = ref(voyData[0]['secVisa'])
  const payment = ref(0)
  ...
  const handleSelectionChange = (travellers) => {
    const addedTravellers = travellers.filter(traveller =>
        !chooseTravellers.value.some(item => item.passportNo === traveller.passportNo)
    );
    const removedTravellers = chooseTravellers.value.filter(traveller =>
        !travellers.some(item => item.passportNo === traveller.passportNo)
    );
    chooseTravellers.value = chooseTravellers.value.filter(traveller =>
        !removedTravellers.some(item => item.passportNo === traveller.passportNo)
    ).concat(addedTravellers.map(traveller => ({
  ...
  const changeType = (row) => {
    if(row.ticketType === 1) row. tickCost = 0
      ...
      if(row.seatType === 0) row. tickCost = voyData[0]['vipVisa']
      else if(row.seatType === 1) row. tickCost = voyData[0]['firVisa']
      else row. tickCost = voyData[0]['secVisa']
    }
  }
   //展示
  ...
  const generateOrder = () => {
    ...
    chooseTravellers.value.forEach(traveller => {
      ...
      traveller['startDate'] = voyData[0]['voyDate'] + ' ' + voyData[0]['startTime']
      traveller['startVoyNo'] = voyData[0]['voyNo']
      ...
      traveller['travelType'] = voyData[0]['startPort'] === 0 ? '散客出境' : '散客入境'
    ...
    service({
      ...
      response => {
        if(response.data.code === 0) {
          ...
          orderData(chooseTravellers.value)
          nextStep()
        }
      }
    )
